...
### [CRITICAL] Stale host node_modules shadowed fresh installs in prod images (found + fixed this round)
**Where:** repo root (no `.dockerignore` existed); `Dockerfile:12-16` (`COPY backend/ .` after `npm ci`); Frankfurt `backend/node_modules` (163 stale pkgs) + `frontend/node_modules` (116)
**Evidence:** `npm audit fix` bumped qs (body-parser 1.20.8/nested qs 6.16.0, lockfile md5-identical locally and on Frankfurt), yet a fresh `--no-cache` image still served body-parser 1.20.6 — clean-room `npm ci` from the same lockfile yields 1.20.8, and the image's own `/app/package-lock.json` (md5 `66f290...`, identical) pins 1.20.8. Root cause: host-side stale `node_modules` (never synced, never cleaned — rsync excludes it) were `COPY`'d over the fresh tree because no `.dockerignore` existed, file-by-file producing a franken-tree no lockfile describes. Every past deploy shipped this mix: lockfile security fixes were silently neutralized in production.
**Impact:** supply-chain integrity — prod dependency tree diverged from the audited lockfile; security patches (qs) did not take effect; a stale duplicate `backend/Dockerfile` + a divergent `backend/vapid-keys.json` (md5 differs from the mounted canonical one) were also baked into image layers.
**Fix:** new `.dockerignore` (node_modules, dist outputs, .git, .env*, vapid-keys*, backups, test/), deleted stale `backend/Dockerfile`, both `node_modules` dirs and the stray `backend/vapid-keys.json` (runtime bind-mounts remain the single source), rebuilt + redeployed. Verified in the final image (`docker run --rm`, no mounts): body-parser **1.20.8**, no vapid keys, no Dockerfile, no stray trees. Suites re-run green on the final image (83/83 + 60/60 + 19/19).
**Residual:** `/app/nodemon.json` still baked (tiny dev config, harmless).

## Feature round 2026-09-17/18 (AI providers, links, intervals, Android, UI proof)

**AI provider situation (verified, not assumed):** b.ai `personal_balance: 0` blocks ALL 48 catalog models (probed each family; qwen3.8-flash included — its "free" appearance was a parameter-validation error firing before the balance check). No code can fix zero balance.
**Fix shipped:** multi-provider chain in `lib/assistant.js` — TokenHarbor (paid primary, `deepseek-v4.1-flash`) → b.ai → Gemini free tier (OpenAI-compat). `QUOTA_EXHAUSTED` error code with honest UX copy; 429 throttling classified transient (never poisons a provider); 150s overall deadline reachable (was dead code); pinned-model provider jumps first; `/meta/models` + settings validation cover all providers via `availableModels()`. Keys via env only (`TOKENHARBOR_*/GEMINI_*` in compose + `.env.example`), never logged/committed.
**Gemini free tier (live-proven, then quota-burned):** past-questions + multilingual (DE/EN/FR/ES) verified working through `gemini-2.5-flash`; single + parallel tool calls work. Free key quota is ~20 req/window SHARED across all users — verification volume plus real use exhausts it within minutes (confirmed 429s, mechanism `generate_content_free_tier_requests`). `gemini-2.5-flash-lite` 404s on the OpenAI-compat layer (removed from defaults). Outstanding until a healthy window or funded provider: multi-create completion proof, link_read-via-chat proof, full adv-suite green.
**Prompt bug found by bisection:** "state exactly what you will do" made Gemini announce instead of calling tools (glm obeyed it as intended). Rephrased to "CALL the matching tool immediately" — calls flow again. Kept confirm-flow intent.
**TokenHarbor status:** key valid (`/models` 200, 35 models incl. `deepseek-v4.1-flash`), but `balance_zero ($0)` on chat — needs top-up at tokenharbor.ai/dashboard to become the DeepSeek primary.
**link_read tool (shipped, LLM-independent parts verified):** SSRF-safe fetch (DNS-resolve + private-range block incl. decimal/octal forms, redirect re-validation, 15s/200KB caps), HTML→text, YouTube oEmbed + shorts/youtu.be/embed/live URL shapes. 10/10 injection probes blocked live; real pages + videos extracted. Throws converted to `{error}` results.
**Intervals complete:** habit every-N verified on ALL surfaces (create/edit API + validation, scheduler live firing, with-scheduled, /scheduled, grid, grid/day, streaks perfect/broken/off-day, assistant create). Task intervals ADDED (`Task.intervalDays`, scheduler, due-ness, stays-active, assistant param, modal/edit UI with badges, sweep coverage). Habit schedule badges (`Every N days`, Daily, Weekdays…) visible on cards.
**Web UI proof (new):** Playwright + Chromium installed locally; dev app screenshotted and visually reviewed. Found + fixed by seeing: assistant input bar floated mid-screen on short content (now fixed-docked), bare `/profile` cold-load bounced to dashboard (vue-router does NOT await async `redirect` fns — Invalid redirect; moved to `beforeEnter`), task Repeat UI + history month grid verified visually (fixed "every N days" copy bug). Zero console errors, zero horizontal overflow on all screens.
**Android (separate repo, no push access — local commits only):** baseline `assembleDebug` green (both modules). Shipped: EncryptedSharedPreferences token stores + migration (app + wear), in-memory token cache (no runBlocking), release-only HTTP logging, `allowBackup=false`, server logout calls, task TimePicker + repeat/interval/reminder wiring, buddy/challenge search+attach, Terms/Privacy/Imprint browser links, Login server field, stay-checkbox wiring, admin announcements POST, reset-password + friend-accept/challenge-invite screens, deep-link intent plumbing, camera proof capture (gallery kept), one-tap proof bypass closed (row + detail), invite double-call fix, default-emoji purge (picker stays), pushed-aware inbox/worker, gradle comdup check (false alarm: -ktx is distinct). Two builds green. Emulator boot attempted (KVM segfault under containerization; software fallback booting — runtime smoke test pending).
**Suites on final prod build:** 83/83 + 69/69 sweep (incl. new PUT-interval regression tests after catching a real `ReferenceError` the suite initially missed) + 19/19 assistant (pre-quota-burn; post-burn runs fail on quota, not code).

### Notification dedupe + layout-shift round (2026-09-11, live)
- **Dedupe:** new `Notification.pushed` flag; `sendPushNotification` returns delivered count; central `notifyUser()` funnel (row → push → flag) replaced all dual-write sites (scheduler reminders, friend requests, challenge invites, admin announcements/test-send). No subs or total push failure → `pushed=false`, in-app stays the fallback. Alerts filter `!pushed`, unread count excludes pushed, list dims pushed rows (actions still available). `userAgent` stored per subscription (future per-device prefs). SW skips OS notification when an app window is focused.
- **Layout shift:** InstallPrompt + PushPrompt moved from late-mounting in-flow cards to a shared fixed bottom stack (zero in-flow height); InstallPrompt now app-wide; Notifications action rows keep `min-h` when read (invisible, not removed); skeleton loading list; alerts leave/move transitions + keyed header; PushPrompt label/hint fixed heights.
- **Verified on dev:** delivered→pushed+badge-clean, failed→stays unread, no-subs→visible; 83/83 + 60/60. **Prod:** `pushed` column live, 83/83 + 60/60 green, new bundle live.
- **b.ai outage 2026-09-11 ~15:2x UTC:** all model calls timing out upstream (DNS+TCP fine, `/chat/completions` hangs; direct 30s probe from prod container also timed out). Not caused by this deploy — fallback chain + user-facing error behaved as designed. Assistant e2e re-run pending provider recovery.

### Notification "off means off" round (2026-09-11, user-reported)
- **Root cause (proven with prod data):** user's morning/evening toggles were OFF (respected — zero digests in 48h), but `habitRemindersEnabled` was ON (untouched since 2026-08-01), so timed-habit reminders kept firing. Two traps found: (1) no master switch — four scattered toggles, none obviously covering "everything"; (2) `PushPrompt.vue` **silently force-set `habitRemindersEnabled=true`** whenever push was enabled — turning settings back on behind the user's back.
- **Fix:** new "All notifications" master switch (Profile, flips all four server toggles); PushPrompt now ASKS via confirm dialog instead of silently re-enabling; habit-reminder label clarified ("Reminders at your habit times"); user's own `habitRemindersEnabled` set to false per explicit request (one-tap reversible).
- **Verified:** 83/83 prod suite, new bundle live.
