...
**Gemini free tier (live-proven, then quota-burned):** past-questions + multilingual (DE/EN/FR/ES) verified working through `gemini-2.5-flash`; single + parallel tool calls work. Then free-tier cap hit (`limit: 20` requests, rolling; my ~60 verification calls today burned it — mostly my testing volume, sorry). `gemini-2.5-flash-lite` 404s on the OpenAI-compat layer (removed from defaults). Outstanding until reset: full adv-suite green + link_read-via-chat + multi-create completion on Gemini.
