<template>
  <div class="min-h-[100dvh]">
    ...
    <nav v-if="!publicRoutes.includes(route.name)" class="border-b border-[var(--app-nav-border)] bg-[var(--app-nav-bg)] backdrop-blur-xl sticky top-0 z-50 safe-top">
      <div class="max-w-5xl mx-auto px-4 h-12 grid grid-cols-[1fr_auto_1fr] items-center gap-2">
        <router-link to="/" class="flex items-center gap-2 font-bold text-lg justify-self-start min-w-0">
          <Logo :size="28" />
          <span class="hidden sm:inline bg-gradient-to-r from-emerald-400 to-emerald-300 bg-clip-text text-transparent">BeBetter</span>
        </router-link>
        <div class="hidden md:flex items-center gap-0.5 justify-self-center">
        ...
        <div class="flex items-center gap-1 justify-self-end shrink-0">
